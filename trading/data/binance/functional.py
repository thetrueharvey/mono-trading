...
from .constants import DEFAULT_START_TIME, INTERVAL, BASE_URL
...
async def update_kline_data(
    ...
    interval: INTERVAL = INTERVAL.INTERVAL_1_MINUTE
):
    with Loader(f"Fetching all data for {symbol} | {interval}..."):
        ...
        try:
            kline_df = pl.read_parquet(
                source=(save_path / f'{interval}' / f'{symbol}').with_suffix(".parquet")
            )
            start_time = str(datetime.fromtimestamp(kline_df["Close time"].cast(pl.Int64).max() / 1e3) + timedelta(milliseconds=1)) + ".000000"
        ...
        if kline_df_ is not None:
            (interval_folder := (save_path / f'{interval}')).mkdir(exist_ok=True)
            ...
            (
                kline_df
                    ...
                    .distinct()
                    .write_parquet(
                        file=(interval_folder / f'{symbol}').with_suffix('.parquet'),
...
async def get_or_update(
    ...
    intervals: Union[INTERVAL, List[INTERVAL]] = [INTERVAL.INTERVAL_1_MINUTE],
):
    ...
    if symbols is None:
        ...
        exchange_df.write_parquet(file=exchange_save_path, compression='uncompressed', use_pyarrow=False, statistics=True)
